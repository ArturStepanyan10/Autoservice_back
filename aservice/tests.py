...
from aservice.models import Appointment, Car, Post, Reviews, Service, Specialization, User, Worker
...
class WorkerModelTest(TestCase):
    ...
    def test_create_worker(self):
        worker = Worker.objects.create(
            user=self.user, specialization=self.specialization, position=self.position, experience=5
        )
...
class AppointmentModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="client@example.com", password="1234", role="Клиент", username="test"
        )
        ...
        self.service = Service.objects.create(
            title="Шиномонтаж", description="Замена резины", labor_time=datetime.timedelta(minutes=60)
        )
...
class ReviewsModelTest(TestCase):
    def setUp(self):
        ...
        self.service = Service.objects.create(
            title="Мойка", description="Мойка кузова", labor_time=datetime.timedelta(minutes=30)
        )

    def test_create_review(self):
        review = Reviews.objects.create(user=self.user, service=self.service, rating=5, content="Отлично всё сделали!")
